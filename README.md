...
- **Grammar.from_bnf_text(bnf_text)`**: Parses BNF rules like `<S> ::= <S>+<S> | x | y`. Tokenization handles adjacent symbols (e.g., `<S>+<S>`).
...
- **EvolutionaryAlgorithm**
  - `__init__(grammar, objective_function, population_size=20, complexity_coefficient=0.1)`: Stores the grammar, objective, population size, and a linear complexity penalty multiplier.
  - `_evaluate(individual)`: Calls the objective, adds length-based penalty, and stores `fitness` (lower is better).
...
- **trading_objective(phenotype_string)**: Executes the generated code in a sandboxed namespace containing `Strategy`, `SMA`, `crossover`, and `np`; runs a backtest on `GOOG` with `Backtest`; returns a penalty or negative Sortino Ratio (to minimize) as fitness.
